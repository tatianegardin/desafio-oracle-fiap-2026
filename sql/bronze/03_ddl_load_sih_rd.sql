...
-- Arquivos RDSPaamm.csv, convertidos de .dbc por etl/conversao/
-- ============================================================
-- 114 colunas, layout oficial RD, todas VARCHAR2 de proposito:
-- a Bronze e 1:1 com a fonte; conversao de tipos acontece na
-- Prata (etl/pipeline/prata_transform.py).
--
-- Pre-requisitos: sql/setup/01_credencial.sql executado e os
-- arquivos RDSP*.csv presentes no bucket hospcheck-staging.
-- ============================================================
...
-- ------------------------------------------------------------
-- Carga: o curinga RDSP*.csv carrega todas as competencias
-- presentes no bucket de uma vez.
-- ATENCAO: COPY_DATA faz APPEND. Para recarregar, rode antes:
--   TRUNCATE TABLE brz_sih_rd_raw;
-- ------------------------------------------------------------
BEGIN
...
-- ------------------------------------------------------------
-- Validacao
-- Referencia: competencia 202601 = 237.622 AIHs no estado,
-- sendo 59.860 na capital (munic_mov = 355030).
-- ------------------------------------------------------------
SELECT ano_cmpt,
       mes_cmpt,
